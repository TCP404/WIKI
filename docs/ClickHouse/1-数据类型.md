...
|--------|------|---------------------|----------------------|
| Int8   | 1    | TinyInt, Bool, Int1 | [-2^8^ : 2^8^-1]     |
| Int16  | 2    | SmallInt, Int2      | [-2^16^ : 2^16^-1]   |
| Int32  | 4    | Int, Int4, Integer  | [-2^32^ : 2^32^-1]   |
| Int64  | 8    | BigInt              | [-2^63^ : 2^63^-1]   |
...
Decimal 是有符号的定点数，可以在加减乘算法过程中保持精度。支持已下写法：
...
### Datetime
Datetime 类型不进不进存储日期，还存储了时间信息。在底层其实都是以 UNIX 时间戳存储。
...
[Official Document](https://clickhouse.com/docs/en/sql-reference/data-types/decimal)
