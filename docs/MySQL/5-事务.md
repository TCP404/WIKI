...
- 幻读是在同一事务中，相同条件下，两次查询出来的 **记录数** 不一样，强调的是 **数量**、增删操作；
- 不可重复读是在同一事务中，相同条件下，两次查询出来的 **数据** 不一样，强调的是 **数据**、修改操作；
- 脏读是读到了未提交的数据。

## 事务隔离级别

!!!note
    对于上述问题，SQL 标准提出了四种隔离级别来规避这些问题，分别是： **<font color='#004d40'>读未提交</font>**、**<font color='#00796b'>读已提交</font>**、**<font color='#00897b'>可重复度</font>**、**<font color='#4db6ac'>串行化</font>**。
...
ReadView 是 InnoDB 用来实现 MVCC 的重要数据结构，用于在不同的事务隔禋级别下，提供不同的读取数据的方式。
