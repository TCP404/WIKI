...
```go
t1 := time.Now()
fmt.Println(t1) // 2021-02-08 11:54:51.5596954 +0800 CST m=+0.001962701
```
...
fmt.Println(t4.Unix())	    // 3600，相差1小时，也就是3600秒
fmt.Println(t4.UnixNano())	// 3600 000 000 000，相差1小时，也就是36万亿纳秒
```

### 当前时间戳 time.Since()、time.Until()
函数签名：
```go
...
fmt.Println(t3)             // 2021-02-08 00:00:00 +0000 UTC
fmt.Println(time.Since(t3)) // 8h21m9.0061743s = 当前时间 - t3
fmt.Println(time.Until(t3)) // -8h21m9.0061743s = t3 - 当前时间
```
...
>   注意：**字符串模板的日期时间必须是 2006年1月2日 15点04分05秒，这是Go语言诞生的时间。（官方彩蛋）**
>
>   记忆方法：6-1-2-3-4-5
...
fmt.Println(s1) // 2021年2月8日 11:56:27

s2 := t2.Format("2006-1-2")
fmt.Println(s2) // 2021-10-1
```
...
if err != nil {
	fmt.Println(err) // parsing time "2021/2/8" as "2006-1-2": cannot parse "/2/8" as "-"
}
fmt.Println(t3)         // 2021-02-08 00:00:00 +0000 UTC
fmt.Printf("%T \n", t3) // time.Time
```
...
>   `Timer` 是**一次性**的时间触发事件，`Ticker` 是按一定时间间隔**持续**触发时间事件。
...
- **`NewTimer()`**
    ...
    func main() {
    	...
    	fmt.Println(time.Now()) // 2021-02-14 17:58:28.7661805 +0800 CST m=+0.003483101
    
    	// 启动定时器，阻塞3秒后继续执行
    	fmt.Println(<-timer.C)  // 2021-02-14 17:58:31.7657332 +0800 CST m=+3.003035801
    }
    ```
...
- **`After()`**
    ...
    func main() {
    	...
    	fmt.Println(time.Now()) // 2021-02-14 18:21:23.3943073 +0800 CST m=+0.003990801
    	// 启动定时器
    	fmt.Println(<-ch) // 2021-02-14 18:21:26.3929503 +0800 CST m=+3.002633801
    }
    ```
- **`AfterFunc()`**
    ```go
...
func main() {
	...
	go func() {
		defer wg.Done()
		<-timer.C    // 启动定时器，定时器结束后（阻塞等待时间结束）打印
		fmt.Println("Timer2 is over.")
	}()

    // 3秒后停止 timer
	time.Sleep(3 * time.Second)
	flag := timer.Stop()    // 提前停止定时器
	if flag {
